...
看板的信息架构、视觉布局、核心组件和交互已通过可运行原型定稿，后续实现应以本蓝图第 16 章和 `reference/prototype/` 为基准。在 Markdown 专项讨论完成前，实现不得擅自固化对应文档模板。其他章节均按当前已收敛方案执行；后续如有变更，应同步修订本蓝图。
...
`reference/prototype/` 是已经通过多轮页面审阅形成的交互原型，用于约束第一版看板的信息架构、视觉关系、密度和交互。原型中的模拟数据、Vinext 工程结构和演示性实现不直接构成最终运行时要求；正式开发时可以抽取为更轻量的固定前端资源，但不得无理由改变本节已经定稿的产品行为。

#### 16.6.1 页面骨架与区域比例

- 看板为桌面优先的单页面应用，占满整个视口，页面本身不产生纵向滚动；目标列表、各任务状态列、时间线和文档阅读区按需独立滚动。
- 页面最小可用宽度为 `980px`。低于该宽度时允许产生整体横向可视限制，第一版不要求重排成移动端卡片流。
...
- Markdown 渲染需支持标题、段落、列表、表格、代码块、引用、链接等基础内容，并继续遵守第 16.4 节的安全过滤要求。
...
- `config.locale` 决定 CLI 与看板界面语言；找不到目标语言资源时回退到项目规定的默认语言，并明确提示回退结果。
- JSON 中的用户数据和 Markdown 正文保持原文，不自动翻译。
- 当前定稿原型使用 `zh-CN` 演示，正式实现需将中文文案从组件代码中剥离为可替换语言资源。
...
- 第一版继续保持只读；原型中的所有按钮和交互不得演变为绕过 CLI 的数据写入入口。
...
- 亮色与暗色主题、`config.locale` 文案切换和缺失语言回退。
...
- 将固定中文界面文案拆分为受 `config.locale` 控制的语言资源。
...
看板页面设计已经通过 `reference/prototype/` 定稿，并在第 16.6 节中形成第一版实现要求，不再列为待决事项。

除 Markdown 文档规范外，本文档其余要求已经作为 GoGoal 第一版开发基线收敛。
